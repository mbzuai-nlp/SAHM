"""
Evaluator for the Fatwa QA dataset.

- Takes a model predictions file (CSV/JSONL/Parquet) with columns: id, split, model_answer
- Merges with ground-truth data (SahmBenchmark/fatwa-qa-evaluation) by id
...
2) Conditions, exceptions, constraints (0–2):
   - Does it retain critical restrictions, qualifiers, or carve-outs that materially affect the ruling (e.g., allowance only if cooperative takaful, prohibition if fee is interest, allowance for adult son not on father’s legal maintenance, etc.)?
...
5) Directness & fatwa format (0–1):
   - Directly answers the question; avoids long digressions; phrasing suitable for a fatwa (concise ruling first or clearly stated).
...
- hallucinated_citations: Does it assert sources (Qurʾān/hadith/fiqh bodies) in a misleading or fabricated way that could distort the ruling?
...
- Compare to ground_truth: we want alignment with the reference ruling and constraints.
...
def load_ground_truth(split: str) -> pd.DataFrame:
    ds = load_dataset("SahmBenchmark/fatwa-qa-evaluation", split=split)
    df = ds.to_pandas()
    for col in ["id", "prompt", "answer", "category"]:
        if col not in df.columns:
            raise ValueError(f"Ground-truth missing required column: {col}")
    return df[["id", "prompt", "answer", "category"]]
...
class GeminiJudge:
    def __init__(self, api_key: str, cfg: JudgeConfig):
        ...
        self.model = genai.GenerativeModel(
            ...
            system_instruction="You are a precise, rigorous evaluator for Arabic fatwa QA. Output only valid JSON.",
            generation_config={
    ...
    def create_evaluation_prompt(
        ...
        """Build the fatwa evaluation prompt. Arabic data fields; English rubric."""
        note_lang_str = "Arabic" if self.cfg.note_language.lower().startswith("arab") else "English"
        rubric = RUBRIC_EN_FATWA.replace("{NOTE_LANG}", note_lang_str)
...
def run(
    ...
    base = f"{split}__gemini25flash_fatwa__judged__{stamp}"
...
def main():
    parser = argparse.ArgumentParser(description="Gemini 2.5 Flash judge for Fatwa QA.")
    ...
    parser.add_argument("--out-dir", type=str, default="research/Finance/outputs/judgments_fatwa")
